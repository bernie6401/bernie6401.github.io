...
- 之前以為有 `SHSTK` 所以 ROP 不能用，但最後 AI 還是找到一個可以用的 ROP Chain ，因為 `SHSTK` 被 `glibc` 關掉了，為了程式相容性
...
- [lincemorado97/CVE-2025-64446](https://github.com/lincemorado97/CVE-2025-64446_CVE-2025-58034/): 有 14 Star ，也有可能是機器人或是買的，存在於 Fortinet 旗下網頁應用程式防火牆 FortiWeb 的重大安全漏洞
...
2. 下載之後會執行 Script ，但 Script 本身沒有直接包含惡意程式
3. 利用 python script install 惡意的 dependency
4. 第一個 dependency 會宣告另外一個 dependency ，所以會再 pull second dependency
5. 第二個 dependency 會 import malicious library
...
2. 需使用者手動切分頁，在實際攻擊時，不能期待受害者自己切分頁 → 避免使用者限制
3. 檢查程式碼: 確定當前 Context 存在已登入 Profile → 或是就算無登入也可以攻擊
